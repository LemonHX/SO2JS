...
impl GcHeader {
    ...
    /// Alignment of allocations (header repr C, expected to be 8 on 64-bit)
    ...
    pub fn layout_for_size(size: usize) -> Layout {
        debug_assert_eq!(core::mem::align_of::<GcHeader>(), Self::ALIGN);
        let total_size = align_up(Self::SIZE + size, Self::ALIGN);
...
mod tests {
    ...
    fn test_gc_header_size() {
        // Ensure GcHeader is properly aligned and sized to a multiple of its alignment
        assert_eq!(GcHeader::SIZE % GcHeader::ALIGN, 0);
        assert_eq!(core::mem::align_of::<GcHeader>(), GcHeader::ALIGN);
    }
